...
describe('one PGlite process per data directory',()=>{
 ...
 it('blocks a second database before initialization and preserves the first connection',async()=>{
  ...
  const reopened=await openDatabase(root);try{expect((await reopened.query('SELECT count(*)::int AS count FROM schema_migrations'))[0].count).toBe(8);}finally{await reopened.close();}
 },30000);
});
