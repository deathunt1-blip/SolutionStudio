...
export class KnowledgeService {
 ...
 async upload(file:SourceFile,options:{duplicate:'skip'|'keep';sourceId?:string;documentId?:string}) {
  ...
  const result=await this.db.transaction(async tx=>{
   ...
   if(!doc) {
    ...
    await tx.query("INSERT INTO documents(id,organization_id,workspace_id,source_document_id,title,status) VALUES($1,'default','default',$2,$3,'uploaded')",[documentId,sourceDocumentId,file.meta.filename]);
   } else {
 ...
 private async process(job:any) {
  ...
  await this.db.query('UPDATE document_versions SET parsed_document=$1::jsonb,parse_status=$2,parse_warnings=$3::jsonb,error=NULL WHERE id=$4',[JSON.stringify(parsed),parsed.parseStatus,JSON.stringify(parsed.parseWarnings),job.version_id]);
  ...
  await this.db.transaction(async tx=>{
   const docs=await tx.query('SELECT user_overrides,title_user,active_version_id,status FROM documents WHERE id=$1',[job.document_id]);
   ...
   await tx.query('UPDATE document_versions SET summary=$1,review_reasons=$2::jsonb,parse_warnings=$3::jsonb WHERE id=$4',[output.summary,JSON.stringify(reasons),JSON.stringify(warnings),job.version_id]);
   if(!docs[0].title_user && parsed.title) await tx.query('UPDATE documents SET title=$1 WHERE id=$2',[parsed.title.slice(0,500),job.document_id]);
 ...
 async edit(id:string,input:Record<string,unknown>) {
  ...
  await this.db.transaction(async tx=>{
   ...
   if(input.title!==undefined) {if(typeof input.title!=='string'||!input.title.trim()||input.title.length>500) throw new HttpError(400,'标题需为 1–500 字');await tx.query('UPDATE documents SET title=$1,title_user=true WHERE id=$2',[input.title.trim(),id]);}
   ...
   if(parsed?.plainText) await this.writeChunks(tx,id,document.activeVersionId,chunkDocument(parsed,classification,{targetTokens:settings.chunkTargetTokens,overlapTokens:settings.chunkOverlapTokens}),classification,String(input.title||document.title));
