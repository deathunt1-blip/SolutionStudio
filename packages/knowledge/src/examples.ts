...
export async function retrieveConfirmedExamples(db:Connection,text:string,excludeId:string,additionalExcludedIds:string[]=[]):Promise<ConfirmedExample[]> {
 ...
 const rows=await db.query(`SELECT e.* FROM confirmed_examples e JOIN documents d ON d.id=e.document_id WHERE ${scoped} AND e.organization_id='default' AND e.workspace_id='default' AND e.scope='global' AND d.status<>'archived' AND e.document_id<>$1${exclusion} AND e.search_vector @@ to_tsquery('simple',$2) ORDER BY ts_rank_cd(e.search_vector,to_tsquery('simple',$2)) DESC,e.created_at DESC LIMIT 5`,additionalExcludedIds.length ? [excludeId,query,additionalExcludedIds] : [excludeId,query]);
