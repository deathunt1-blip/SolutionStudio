...
export type DocumentBlock =
 ...
 | {id:string;type:'diagram';diagramType:'mermaid';source:string;caption?:string;generatedBy:'ai'|'user';sourceRefs:SourceRef[]}
 | {id:string;type:'callout';kind:'设计说明'|'关键参数'|'工程依据';text:string};
export interface VisualPlan {type:'none'|'scenelab'|'mermaid'|'knowledge_image'|'user_upload'|'ai_illustration';description?:string;assetId?:string}
export interface DocumentTemplateSection {id:string;title:string;level:number;order:number;generationMode:'fixed'|'ai'|'mixed'|'table'|'asset';requiredContext:string[];retrievalPolicy?:{query:string;authority?:string;limit?:number};generationInstruction?:string;fixedContent?:string;condition?:'engineering';assetRoles?:string[];visualPlan?:VisualPlan;tableKind?:'requirements'|'equipment'|'products'|'engineering';validationRules?:string[]}
...
export interface Claim {text:string;factIds:string[];sourceIds:string[];kind?:'requirement'|'capability'|'engineering'|'principle'}
export interface OutputProfile {companyName:string;fontFamily:string;fontSize:number;titleFontFamily:string;pageMarginMm:number;header:string;footer:string;coverLogoAssetId?:string;themeId?:string;brandColor?:string}
...
export interface ValidationIssue {id:string;sectionId:string;blockId?:string;severity:'error'|'warning';type:'fact_mismatch'|'unsupported_claim'|'requirement_conflict'|'missing_source'|'stale_context'|'incomplete'|'customer_facing'|'prohibited_claim'|'layout';message:string;quote?:string;sourceRefs:SourceRef[]}
...
export type GenerationMode='regenerate'|'shorten'|'expand'|'formal'|'technical'|'rewrite'|'diagram';
