...
function fontProperties(family: string, size: number, bold = false): string {
  return `<w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman" w:eastAsia="${escapeXml(family)}" w:cs="Times New Roman"/><w:color w:val="000000"/><w:sz w:val="${Math.round(size * 2)}"/><w:szCs w:val="${Math.round(size * 2)}"/>${bold ? '<w:b/><w:bCs/>' : ''}`;
}
function stylesXml(profile: OutputProfile): string {
  const body = fontProperties(profile.fontFamily, profile.fontSize);
  ...
  return XML + `<w:styles xmlns:w="${W}"><w:docDefaults><w:rPrDefault><w:rPr>${body}<w:lang w:val="en-US" w:eastAsia="zh-CN"/></w:rPr></w:rPrDefault></w:docDefaults>`
    + paragraphStyle('Normal', 'Normal', '<w:widowControl/><w:spacing w:after="120" w:line="360" w:lineRule="auto"/>', body)
    + paragraphStyle('Title', 'Title', '<w:keepNext/><w:spacing w:before="1200" w:after="400"/><w:jc w:val="center"/>', fontProperties(profile.titleFontFamily, 28, true))
    + [1, 2, 3].map(level => paragraphStyle(`Heading${level}`, `heading ${level}`, `<w:keepNext/><w:keepLines/><w:spacing w:before="${level === 1 ? 320 : 240}" w:after="160"/><w:outlineLvl w:val="${level - 1}"/>`, fontProperties(profile.titleFontFamily, [18, 15, 12][level - 1], true))).join('')
    + paragraphStyle('TOCHeading', 'TOC Heading', '<w:keepNext/><w:spacing w:after="280"/>', fontProperties(profile.titleFontFamily, 18, true))
    + [1, 2, 3].map(level => paragraphStyle(`TOC${level}`, `toc ${level}`, `<w:spacing w:after="120"/><w:ind w:left="${(level - 1) * 360}"/>`, fontProperties(profile.fontFamily, profile.fontSize, level === 1))).join('')
    + paragraphStyle('Caption', 'Caption', '<w:keepLines/><w:spacing w:before="100" w:after="160"/><w:jc w:val="center"/>', fontProperties(profile.fontFamily, 10))
    + paragraphStyle('TableText', 'Table Text', '<w:spacing w:after="40" w:line="280" w:lineRule="auto"/>', fontProperties(profile.fontFamily, Math.min(profile.fontSize, 10.5)))
    + paragraphStyle('Header', 'header', '<w:spacing w:after="0"/><w:jc w:val="right"/>', fontProperties(profile.fontFamily, 9))
    + paragraphStyle('Footer', 'footer', '<w:spacing w:after="0"/><w:jc w:val="center"/>', fontProperties(profile.fontFamily, 9))
    + '</w:styles>';
}
function numberingXml(listCount: number): string {
  const headingLevels = [0, 1, 2].map(level => `<w:lvl w:ilvl="${level}"><w:start w:val="1"/><w:numFmt w:val="decimal"/><w:lvlText w:val="${Array.from({ length: level + 1 }, (_, i) => `%${i + 1}`).join('.')}"/><w:suff w:val="space"/><w:lvlJc w:val="left"/></w:lvl>`).join('');
...
function tableXml(block: Extract<DocumentBlock, { type: 'table' }>, width: number): string {
  ...
  const row = (values: string[], index: number) => `<w:tr>${index === 0 ? '<w:trPr><w:tblHeader/></w:trPr>' : ''}${values.map((value, column) => {
    ...
    return `<w:tc><w:tcPr><w:tcW w:w="${widths[column]}" w:type="dxa"/><w:vAlign w:val="center"/>${index === 0 ? '<w:shd w:fill="DCE6F1"/>' : ''}</w:tcPr>${p(run(value, index === 0 ? '<w:b/>' : ''), style('TableText') + (center || index === 0 ? '<w:jc w:val="center"/>' : ''))}</w:tc>`;
  }).join('')}</w:tr>`;
  return `<w:tbl><w:tblPr><w:tblW w:w="${width}" w:type="dxa"/><w:tblLayout w:type="fixed"/><w:tblBorders>${borders}</w:tblBorders><w:tblCellMar><w:top w:w="100" w:type="dxa"/><w:left w:w="120" w:type="dxa"/><w:bottom w:w="100" w:type="dxa"/><w:right w:w="120" w:type="dxa"/></w:tblCellMar></w:tblPr><w:tblGrid>${widths.map(value => `<w:gridCol w:w="${value}"/>`).join('')}</w:tblGrid>${row(block.columns, 0)}${block.rows.map((values, i) => row(values, i + 1)).join('')}</w:tbl>`;
}
...
export async function exportDocument(document: GeneratedDocument, options: ExportOptions = { assets: new Map() }): Promise<Buffer> {
  ...
  const profile = profileOf(document.outputProfile);
  const sections = [...document.sections].sort((a, b) => a.order - b.order || a.id.localeCompare(b.id));
  let priorLevel = 0;
  ...
  const dateLabel = `${metaDate.getUTCFullYear()}年${String(metaDate.getUTCMonth() + 1).padStart(2, '0')}月${String(metaDate.getUTCDate()).padStart(2, '0')}日`;
  ...
  if (profile.coverLogoAssetId) {
    const logo = embedImage(profile.coverLogoAssetId, Math.min(60, 210 - 2 * profile.pageMarginMm) * emuPerMm, 25 * emuPerMm);
    coverLogo = p(drawing(logo.relationship, ++imageId, `${profile.companyName} Logo`, logo.width, logo.height), centered + '<w:keepNext/><w:spacing w:after="240"/>');
  }
  const cover = coverLogo + p(run(title), style('Title'))
    + p(run('技术方案', fontProperties(profile.titleFontFamily, 20)), centered + '<w:spacing w:after="600"/>')
    + p(run(`项目名称：${document.projectName}`, fontProperties(profile.fontFamily, 13)), centered)
    + (document.customerName ? p(run(`客户名称：${document.customerName}`, fontProperties(profile.fontFamily, 13)), centered) : '')
    + p(run(profile.companyName, fontProperties(profile.fontFamily, 13)), centered + '<w:spacing w:before="1800" w:after="180"/>')
    + p(run(dateLabel, fontProperties(profile.fontFamily, 12)), centered)
  ...
  for (const [sectionIndex, section] of sections.entries()) {
    ...
    toc.push(p(`<w:hyperlink w:anchor="${bookmark}" w:history="1">${run(`${number}  ${section.title}`)}</w:hyperlink>`, style(`TOC${section.level}`)));
    ...
    for (const block of section.blocks) {
      ...
      else if (block.type === 'heading') body.push(p(blockRuns(block), style(`Heading${Math.max(1, Math.min(3, block.level ?? Math.min(3, section.level + 1)))}`)));
      else if (block.type === 'list') {
        const listId = 2 + listCount++ * 2 + (block.ordered ? 1 : 0);
        for (const item of block.items) body.push(p(run(item), `<w:numPr><w:ilvl w:val="0"/><w:numId w:val="${listId}"/></w:numPr>`));
      } else if (block.type === 'table') {
        const caption = `表${chapter}-${++tables}${block.title ? ` ${block.title}` : ''}`;
        body.push(p(run(caption), style('Caption') + '<w:keepNext/>'), tableXml(block, contentWidth), p('', '<w:spacing w:after="60" w:line="100" w:lineRule="exact"/>'));
      } else if (block.type === 'asset') {
        const caption = `图${chapter}-${++figures}${block.caption ? ` ${block.caption}` : ''}`;
        const embedded = embedImage(block.assetId, (210 - 2 * profile.pageMarginMm) * emuPerMm, Math.min(170, 297 - 2 * profile.pageMarginMm - 45) * emuPerMm);
        body.push(p(drawing(embedded.relationship, ++imageId, caption, embedded.width, embedded.height), '<w:keepNext/>' + centered), p(run(caption), style('Caption')));
  ...
  const sectionProperties = `<w:sectPr><w:headerReference w:type="default" r:id="rIdHeader"/><w:footerReference w:type="default" r:id="rIdFooter"/>${pageSetup}<w:pgNumType w:start="1"/></w:sectPr>`;
  const documentXml = XML + `<w:document xmlns:w="${W}" xmlns:r="${R}" xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture"><w:body>${cover}${p(run('目录'), style('TOCHeading'))}${tocField}${p('<w:r><w:br w:type="page"/></w:r>')}${body.join('')}${sectionProperties}</w:body></w:document>`;
  if (Buffer.byteLength(documentXml) > 32 * 1024 * 1024) throw new Error('导出正文超过 32 MB');
  put('word/document.xml', documentXml);
  put('word/styles.xml', stylesXml(profile));
  put('word/numbering.xml', numberingXml(listCount));
  ...
  put('word/header1.xml', XML + `<w:hdr xmlns:w="${W}">${p(run(profile.header), style('Header'))}</w:hdr>`);
  put('word/footer1.xml', XML + `<w:ftr xmlns:w="${W}">${p((profile.footer ? run(`${profile.footer}  `) : '') + run('第 ') + field('PAGE', '1') + run(' 页'), style('Footer'))}</w:ftr>`);
