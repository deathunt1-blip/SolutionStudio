...
export class DocumentEngine {
 ...
 async copy(id:string){const original=await this.get(id),copyId=randomUUID(),assetIds=original.sections.flatMap(section=>section.blocks.filter(block=>block.type==='asset').map(block=>block.assetId));if(original.outputProfile.coverLogoAssetId)assetIds.push(original.outputProfile.coverLogoAssetId);
  await this.db.transaction(async tx=>{await this.checkAssets(tx,original.projectId,assetIds,assetIds);const current=(await tx.query('SELECT revision FROM generated_documents WHERE id=$1 FOR UPDATE',[id]))[0];if(!current)throw new HttpError(404,'方案不存在');if(current.revision!==original.revision)throw new HttpError(409,'原方案已更新，请刷新后重新复制');await this.idle(tx,id);const row=await this.row(id,tx);await tx.query('INSERT INTO generated_documents(id,project_id,template_id,template_version,context_revision,context_snapshot,title,output_profile) VALUES($1,$2,$3,$4,$5,$6::jsonb,$7,$8::jsonb)',[copyId,original.projectId,original.templateId,original.templateVersion,original.contextRevision,json(row.context_snapshot),`${original.title.slice(0,190)}（副本）`,json(original.outputProfile)]);
 ...
 private async process(job:any){
  ...
  const worker=async()=>{while(!this.stopping&&!exhausted){
   ...
   try{
    ...
    const next:DocumentSection={...section,...result,blocks:[...result.blocks,...fixed],sourceRefs:[...new Map(references.map(r=>[r.type+':'+r.id,r])).values()],assetRefs:[...new Set([...result.assetRefs,...fixed.filter(b=>b.type==='asset').map(b=>b.assetId)])],status:'generated',edited:job.mode==='diagram'?section.edited:false,error:undefined,contextTokens:sc.tokens};
