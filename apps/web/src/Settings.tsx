...
export default function Settings({ settings, registries, reload, notify }: { settings?: SettingsType; registries?: Registries; reload: () => void; notify: Notify }) {
  ...
  return <>
    ...
    {tab === 'sources' && (settings ? <SourceSettings settings={settings} /> : <Loading />)}
  </>;
}
...
function ModelSettings({ settings, reload, notify }: { settings: SettingsType; reload: () => void; notify: Notify }) {
  ...
  return <div className="settings-layout model-layout"><aside className="settings-side"><span className="model-mark">K</span><h3>模型负责理解</h3><p>分类引擎通过统一接口连接模型，当前使用 Kimi / Moonshot 兼容接口。</p><div className="settings-tip"><ShieldCheck size={18} /><p>每份资料只提交有限的文档特征用于分类；切块与索引在本地完成，控制调用成本。</p></div><div className="model-state"><span className={`live-dot ${settings.llm.configured ? '' : 'offline'}`} />{settings.llm.configured ? 'API 密钥已配置' : '尚未配置 API 密钥'}</div></aside><form className="panel form-panel" onSubmit={save}><div className="panel-heading"><div><h2>LLM 连接</h2><p>模型只理解原始内容，不补充资料中不存在的事实。</p></div><span className={`status ${settings.llm.configured ? 'status-active' : 'status-needs_review'}`}>{settings.llm.configured ? '已配置' : '未配置'}</span></div><div className="form-panel-body"><label className="field">API Base URL<input type="url" required value={form.baseUrl} onChange={event => setForm({ ...form, baseUrl: event.target.value })} placeholder="https://api.moonshot.cn/v1" /></label><label className="field">API Key<input type="password" value={form.apiKey} autoComplete="new-password" onChange={event => setForm({ ...form, apiKey: event.target.value })} placeholder={settings.llm.configured ? '•••••••••••••••• · 已安全保存' : '输入 API 密钥'} /><small>{settings.llm.configured ? '留空保留现有密钥。只有需要更换时才填写。' : '密钥仅保存在服务器端，不会随设置数据返回。'}</small></label><label className="field">模型名称<input required value={form.model} onChange={event => setForm({ ...form, model: event.target.value })} placeholder="kimi-k2.6" /></label><div className="form-grid"><label className="field">Temperature<input type="number" required min="0" max="2" step="0.1" value={form.temperature} onChange={event => setForm({ ...form, temperature: Number(event.target.value) })} /><small>按模型支持范围设置。</small></label><label className="field">最大输出 tokens<input type="number" required min="256" max="16000" step="1" value={form.maxTokens} onChange={event => setForm({ ...form, maxTokens: Number(event.target.value) })} /><small>仅限结构化分类与摘要输出。</small></label></div>{error && <ErrorMessage message={error} />}{testResult && <div className={`notice ${testResult.ok ? 'success-notice' : 'warning'}`}>{testResult.ok ? <CheckCircle2 size={17} /> : <Unplug size={17} />}<p>{testResult.message}</p></div>}<div className="model-actions"><button type="button" className="button" disabled={!!busy || !settings.llm.configured || dirty} title={dirty ? '请先保存更改后测试连接' : '使用已保存配置发起一次短请求'} onClick={() => void test()}>{busy === 'test' ? <LoaderCircle className="spin" size={15} /> : <Link2 size={15} />}测试已保存的连接</button><button className="button primary" disabled={!!busy}>{busy === 'save' ? <LoaderCircle className="spin" size={15} /> : <Save size={15} />}保存配置</button></div><small className="subtle">{dirty ? '有未保存的更改，请先保存后再测试。' : '连接测试会发起一次简短模型请求。'}</small></div></form></div>;
}

function SourceSettings({ settings }: { settings: SettingsType }) {
  const hasFeishu = settings.sources.some(source => source.type.includes('feishu'));
  return <div className="source-settings"><section className="panel"><div className="panel-heading"><div><h2>知识来源</h2><p>所有来源统一进入解析、分类与索引流程。</p></div><span className="tag">Phase 0</span></div><div className="source-cards">{settings.sources.map(source => <article className="source-card" key={source.id}><span className={`source-icon ${source.type.includes('feishu') ? 'feishu' : ''}`}><FolderInput size={24} /></span><div><h3>{source.name}</h3><p>{source.type.includes('feishu') ? '连接器接口已预留，当前仅用于本地演示。尚未连接真实飞书空间。' : source.type.includes('local') ? '通过目录导入工具读取本地资料，保留来源路径。' : '从收件箱上传单个文件或批量拖入资料。'}</p><div className="source-card-details"><span>模式：{source.mode === 'mirror' ? '镜像来源' : '本地托管'}</span><span>标识：{source.id}</span></div></div><span className={`status ${source.type.includes('feishu') ? 'status-archived' : 'status-active'}`}>{source.type.includes('feishu') ? '预留 / 演示' : ['active', 'connected', 'ready'].includes(source.status) ? '可用' : source.status}</span></article>)}{!hasFeishu && <article className="source-card"><span className="source-icon feishu"><Link2 size={24} /></span><div><h3>飞书知识空间</h3><p>连接器接口已预留。当前版本提供演示适配器，尚未连接真实飞书空间。</p><div className="source-card-details"><span>模式：镜像来源</span><span>后续配置同步目录与授权</span></div></div><span className="status status-archived">尚未接入</span></article>}</div></section><div className="notice"><Database size={18} /><div><strong>知识存储独立于资料来源</strong><p>原文件、历史版本、解析结果和知识索引保存在 Solution Studio。来源连接器可以扩展，知识处理流程保持一致。</p></div></div></div>;
}
