...
工程报告保留每组相机实际采用的镜头、视场角与仿真最大工作距离。与同型号权威产品资料出现差异时，项目理解显示两边的原值及来源；用户可明确选择按报告配置进行工程分析。该选择保留差异记录，不修改通用产品事实，也不把仿真输入视为实测能力。正文和参数表分别标明报告配置与通用产品规格。旧导入数据第一次重建上下文时补读原报告，原包、原图与已有方案快照保持不变。
